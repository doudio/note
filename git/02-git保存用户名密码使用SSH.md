> git 保存用户名密码使用SSH

> 进入指定目录

```cmd
$ cd ~
```

> 创建一个 ssh 目录: $ ssh-keygen -t rsa -C `用户邮箱`

```cmd
Administrator@JavaEE MINGW64 ~
$ ssh-keygen -t rsa -C root@root.com
Generating public/private rsa key pair.
Enter file in which to save the key (/c/Users/Administrator/.ssh/id_rsa):
Enter passphrase (empty for no passphrase):
Enter same passphrase again:
Your identification has been saved in /c/Users/Administrator/.ssh/id_rsa.
Your public key has been saved in /c/Users/Administrator/.ssh/id_rsa.pub.
The key fingerprint is:
SHA256:MDUElmLB/X3FtTUxdY37+aLaeFo4SHm/4i3q2IwDkLM root@root.com
The key's randomart image is:
+---[RSA 2048]----+
|   ..oo++    . *O|
|    +.o. .    + B|
|   o .o. .   . o |
|  +    o... . .  |
|   +    S ..   ..|
|  E .  . o o   ..|
|     .  . o o   .|
|      .=  o*... .|
|      oo=o=*=. . |
+----[SHA256]-----+
```

> 进入 ssh 目录

```cmd
Administrator@JavaEE MINGW64 ~
$ cd .ssh/
```

> 查看文件, 拷贝结果在 git 服务器中添加 SSH Keys

```cmd
$ cat id_rsa.pub
ssh-rsa  root@root.com
```

> 进入工作区, 查看提交的`origin`

```cmd
$ git remote -v
origin  http://www.baidu.com/root/test.git (fetch)
origin  http://www.baidu.com/root/test.git (push)
```

> 添加ssh地址

```cmd
$ git remote add origin_ssh git@www.baidu.com:root/test.git

Administrator@JavaEE MINGW64 /d/git_test/test (test)
$ git remote -v
origin  http://www.baidu.com/root/test.git (fetch)
origin  http://www.baidu.com/root/test.git (push)
origin_ssh      git@www.baidu.com:root/test.git (fetch)
origin_ssh      git@www.baidu.com:root/test.git (push)
```

> 修改 提交
